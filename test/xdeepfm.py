...
# 4 WDL,其中linear_feature_columns只是为继承基类
model = xDeepFM(linear_feature_columns=fixlen_feature_columns, dnn_feature_columns=fixlen_feature_columns,
...
plt.title('xDeepFM loss curve')
plt.show()

plt.plot(history.epoch, history.history['auc'])
plt.title('xDeepFM auc curve')
plt.show()
...
xDeepFM(
  ...
  (cin): CIN(
    ...
    (conv1ds): ModuleList(
      ...
      (1): Conv1d(3328, 128, kernel_size=(1,), stride=(1,))
    )
  )
