...
# 4 PNN训练
model = PNN(fixlen_feature_columns, dnn_activation='relu', \
...
plt.title('PNN loss curve')
plt.plot(history.epoch, history.history['val_auc'])
plt.title('PNN auc curve')
plt.show()
...
PNN(
  ...
  (dnn): DNN(
    ...
    (linears): ModuleList(
      (0): Linear(in_features=442, out_features=128, bias=True)
